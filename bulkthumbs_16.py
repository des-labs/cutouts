...
def run(args):
    ...
    logname = outdir + 'BulkThumbs_' + logtime + '.log'         # use for local
    ...
    for i in tilenm:
        if not TILES_FOLDER or TILES_FOLDER == 'DESLABS-TILES':
            ...
            if TILES_FOLDER == 'DESLABS-TILES':
                ...
                # Make a list of all the unique file name endings we want, like _irg.tiff and _g.fits.fz.
                downloadcolors = []
                if args.make_fits:
                    downloadcolors += colors_fits
                if args.make_rgb_stiff or args.make_rgb_lupton:
                    downloadcolors += colors_rgb
                downloadcolors = (''.join(downloadcolors)).lower()
                downloadcolors = list(set(re.sub('[^grizy]','',downloadcolors)))
                downloadlist = []
                for dc in downloadcolors:
                    if dc == 'y':
                        downloadlist.append('Y.fits.fz')
                    else:
                        downloadlist.append(dc+'.fits.fz')
            ...
            tiledir = os.path.join(TILES_FOLDER, i) + '/'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tiledir', required=False, help='Directory where tiles are stored. Leave this blank and bulkthumbs will download any tile it needs.')
