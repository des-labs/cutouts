...
"""
TESTS:
Options: time mpirun -n 6 python bulkthumbs_10.py --csv des_tiles_sample_135518_coadds.csv --make_pngs --xsize 1 --ysize 1
	6 cores: (ncsa) 

Options: time mpirun -n 6 python bulkthumbs_10.py --csv des_tiles_sample_129411_coords.csv --make_pngs --xsize 1 --ysize 1
	6 cores: (ncsa) 
"""
...
def ScratchRGB():
	"""
	This is the function that can be called for making cutouts from scratch.
	How do you take in all objects that need to be made from scratch so as to do it all at once?
	"""

def MakeLuptonRGB(tiledir, outdir, df, positions, xs, ys, colors, bp, s, q):
	"""
	Assumptions:
		+ each call to this function will have small dataframes, so opening the master tiles per object vs per group of objects may be negligible
	
	- if the outdir does not exist, then make all cutouts from scratch
	- if the outdir does exist, then check number of each color/fits extension and compare to length of input df; else then make all cutouts from scratch
		- then list all files and match them all to the input df to see if there are any unaccounted objects in the df
			- if any unaccounted objects, then make all from scratch; else open each set of files and make it that way
	...
	for i in colors:
		c = i.split(',')
		try:
			file_r = glob.glob(tiledir + '*_{}.fits.fz'.format(c[0].lower() if c[0] != 'Y' else c[0]))
		except IndexError as e:
			print('No FITS file in {0} color band found. Will not create RGB cutout.'.format(c[0]))
			logger.error('MakeLuptonRGB - No FITS file in {0} color band found. Will not create RGB cutout.'.format(c[0]))
			continue
		else:
			r, header = fits.getdata(file_r[0], 'SCI', header=True)
			w = WCS(header)
		try:
			file_g = glob.glob(tiledir + '*_{}.fits.fz'.format(c[1].lower() if c[1] != 'Y' else c[1]))
		except IndexError as e:
			print('No FITS file in {0} color band found. Will not create RGB cutout.'.format(c[1]))
			logger.error('MakeLuptonRGB - No FITS file in {0} color band found. Will not create RGB cutout.'.format(c[1]))
			continue
		else:
			g = fits.getdata(file_g[0], 'SCI')
		try:
			file_b = glob.glob(tiledir + '*_{}.fits.fz'.format(c[2].lower() if c[2] != 'Y' else c[2]))
		except IndexError as e:
			print('No FITS file in {0} color band found. Will not create RGB cutout.'.format(c[1]))
			logger.error('MakeLuptonRGB - No FITS file in {0} color band found. Will not create RGB cutout.'.format(c[2]))
			continue
		else:
		...
		for p in range(len(positions)):
			if 'COADD_OBJECT_ID' in df:
				filenm = outdir + '{0}_{1}{2}{3}.png'.format(df['COADD_OBJECT_ID'][p], c[0].lower(), c[1].lower(), c[2].lower())
			else:
				filenm = outdir + 'DESJ' + _DecConverter(df['RA'][p], df['DEC'][p]) + '_{0}{1}{2}.png'.format(c[0].lower(), c[1].lower(), c[2].lower())
			...
			left = max(0, pixcoords[0][p] - udx)
			upper = max(0, image.size[1] - pixcoords[1][p] - udy)
			right = min(pixcoords[0][p] + udx, 10000)
			lower = min(image.size[1] - pixcoords[1][p] + udy, 10000)
...
if __name__ == '__main__':
	...
	parser.add_argument('--make_rgb', action='append', type=str.upper, help='Creates 3-color images using the bands you select (reddest to bluest), e.g.: --make_rgb i,r,g --make_rgb z,i,r --make_rgb z,r,g')
